...
struct Node {
    ...
    Node* nextSliding;      // Con trỏ đến nút anh/chị kế tiếp

    // Hàm khởi tạo để khởi tạo một nút với giá trị dữ liệu
    Node(int data) : data(data), firstChild(nullptr), nextSliding(nullptr) {}
};
...
private:
    ...
    int calculateHeight(Node* node) {
        ...
        while (child) {
            maxHeight = max(maxHeight, calculateHeight(child)); // Lấy chiều cao tối đa từ các nút con
            child = child->nextSliding; // Chuyển sang nút anh/chị kế tiếp
        }
        return maxHeight + 1;  // Thêm 1 để bao gồm nút hiện tại vào chiều cao
    }

    // Duyệt cây theo thứ tự Preorder: Trước tiên là gốc, sau đó duyệt các nút con
    void preorderTraversal(Node* node) {
        if (node == nullptr) return; // Trường hợp cơ bản: Nếu nút là null, trả về
    ...
    // Duyệt cây theo thứ tự Postorder: Trước tiên duyệt các nút con, sau đó là nút gốc
    ...
    // Duyệt cây theo thứ tự Inorder cho cây nhị phân: nút con trái, gốc, nút con phải
    void inorderTraversal(Node* node) {
        ...
        if (child && child->nextSliding) {
            inorderTraversal(child->nextSliding); // Thăm nút con phải nếu có
        }
    }

    // Kiểm tra xem cây có phải là cây nhị phân không: Mỗi nút phải có tối đa 2 con
...
public:
    ...
    void addChild(int u, int v) {
        ...
        if (parent->firstChild == nullptr) {
            ...
            temp->nextSliding = child; // Thêm nút con vào vị trí anh/chị kế tiếp
        }
    ...
    // Hàm cập nhật nút gốc: Tìm nút không có cha (không phải nút con của bất kỳ nút nào khác)
    void updateRoot() {
    ...
    void displayPreorder() {
        preorderTraversal(root); // Gọi hàm preorderTraversal
        cout << endl;
    }

    // Hiển thị thứ tự duyệt cây theo Postorder
    void displayPostorder() {
        postorderTraversal(root); // Gọi hàm postorderTraversal
        cout << endl;
    }
    ...
    void displayInorder() {
        if (isBinaryTree(root)) {
            inorderTraversal(root); // Gọi hàm inorderTraversal
            cout << endl;
        } else {
            cout << "NOT BINARY TREE" << endl; // Nếu không phải cây nhị phân, in thông báo lỗi
        }
    }
